  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function (position) {
      ...
      $.getJSON('/near/' + position.coords.latitude + '/' + position.coords.longitude, function (data) {
        ...
        //   all = data;
        //   str = 'insert into maps.parkings\n (name, lat, lng, description, a_lots, t_lots, pin, price)\n'
        //   for (var i = 0; i < all.length ; i++){
        //     str += '("' + all[i].name + '",' + all[i].lat + ',' + all[i].lng + ',"' + all[i].description + '",' + all[i].a_lots + ',' + all[i].t_lots + ',"' + all[i].pin + '",' + all[i].price + '), \n';
        //   }
        //   console.log(str);
        // });
